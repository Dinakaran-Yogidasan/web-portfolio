...
const WindowControls = memo(() => (
  <div className="flex gap-1.5">
    {WINDOW_CONTROLS.map((control, i) => (
      <div key={i} className={`w-3 h-3 rounded-full ${control.color}`} />
    ))}
  </div>
));
...
const TechTags = memo(
  ({
    ...
    <div className="flex flex-wrap gap-2">
      {tags.slice(0, maxVisible).map((tech) => (
        <span
          key={tech}
...
const ModalTechTags = memo(({ tags }: { tags: string[] }) => (
  <div className="flex flex-wrap gap-2">
    {tags.map((tech) => (
      <span
        key={tech}
...
ModalTechTags.displayName = "ModalTechTags";
...
const ProjectCard = memo(
  ({
    ...
    const categoryStyle =
      categoryConfig[project.category as keyof typeof categoryConfig]?.badge ||
      "bg-slate-300 text-slate-700";

    return (
      <div className="group relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl md:rounded-3xl overflow-hidden hover:shadow-2xl hover:border-neon-cyan/50 dark:hover:border-neon-cyan/30 transition-all duration-500 flex flex-col h-full hover:-translate-y-2">
        {/* Image Container */}
        {project.imageUrl && (
        ...
        <div className="flex-1 flex flex-col p-5 md:p-6 lg:p-8">
          <div className="flex items-start justify-between mb-4">
            <span
              className={`inline-block px-3 md:px-4 py-1.5 text-xs md:text-sm font-bold rounded-full ${categoryStyle}`}
            >
              {project.category}
            </span>
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="p-6 md:p-10">
            ...
            <div className="mb-8">
              ...
              <ModalTechTags tags={project.tags || []} />
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              {project.liveUrl && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center justify-center gap-2 px-8 py-3 bg-blue-500 text-white rounded-xl font-bold hover:shadow-lg hover:shadow-neon-cyan/50 transition-all duration-300 transform hover:scale-105"
                >
                  <Eye size={20} />
                  View Live Project
                </a>
...
const Projects: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      <ProjectModal
        project={
          (selectedProject as ProjectWithDetails) || portfolioData.projects[0]
        }
